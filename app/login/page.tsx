...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <main className="mx-auto flex w-full max-w-md flex-1 items-center px-6 py-10">
      <form action={loginUser} className="w-full rounded-2xl border border-border bg-card p-6">
        <h1 className="text-3xl font-black text-ball-500">Iniciar sesión</h1>
        <p className="mt-2 text-sm text-text-muted">Accede con el correo y contraseña de tu equipo.</p>
        {params.error && (
